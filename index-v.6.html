...
<head>
  ...
  <meta property="og:title" content="Festive Offer @ B4 Unisex Salon" />
  <meta property="og:description" content="Spin the wheel and grab your discount today!" />
  <meta property="og:image" content="https://b4u-k.github.io/spinning-wheel/button.png" />
  <meta property="og:url" content="https://b4u-k.github.io/spinning-wheel/" />
  ...
  <style>
    ...
    .star{position:absolute;width:12px;height:12px;background:yellow;
...
/* animation */
let spinning=false,startTime=null,duration=4200,finalAngle=0,targetIndex=null;
function easeOutCubic(t){return 1-Math.pow(1-t,3);}
function createStars(){for(let i=0;i<12;i++){const s=document.createElement('div');s.className='star';
  ...
  centerBox.appendChild(s);setTimeout(()=>s.remove(),1000);}}
function animateSpin(ts){
  ...
  spinning=false;pointer.classList.add('winner');
  const discount=discounts[targetIndex];
  centerText.innerText=`You Won: ${discount} Discount 🎉`;createStars();}
centerBox.addEventListener('click',()=>{
  if(spinning)return;spinning=true;pointer.classList.remove('winner');centerText.innerText='B4 Unisex Salon';
  ...
  startTime=null;requestAnimationFrame(animateSpin);
});
</script>
</body>
</html>
